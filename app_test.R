...
runners_finished <- runner_data %>%
  ...
  mutate(
    start_time_seconds = case_when(
      quartile == 1 ~ (73600) + 3060, #7:30am
      quartile == 2 ~ (83600), #8am
      quartile %in% c(3,4) ~ (83600) + 35*60 #8:35am
    ),
...
==============================================================================
  ...
  ARCHIVED ANALYSIS CODE (Wrapped to prevent app.R execution errors)
...
==============================================================================
  ...
  if(FALSE) {
    ...
    heat_incr_table <- tibble(
      ...
      gen_sample_added_minutes = (gen_slopetemp_incr)/60,
      top_1percect_added_minutes = (top1_slopetemp_incr)/60
    )
    ...
    tidy(general_model, conf.int = TRUE, level = 0.95)
    #top 1%:
    elite_model <- lm(finish_time_seconds ~ avg_race_temp_f, data = top_1percent)
    tidy(elite_model, conf.int = TRUE, level = 0.95)
    
    #95% confidence intervals for the regression slopes
    gen_ci <- tidy(general_model, conf.int = TRUE) |>
      filter(term == "avg_race_temp_f")
    
    elite_ci <- tidy(elite_model, conf.int = TRUE) |>
      filter(term == "avg_race_temp_f")
    
    cat(
      "General Sample Slope:",
      round(gen_ci$estimate, 2),
      "sec/°F (95% CI:",
      round(gen_ci$conf.low, 2),
      "to",
      round(gen_ci$conf.high, 2),
      "sec/°F)\n"
    )
    
    cat(
      "Elite Sample Slope:",
      round(elite_ci$estimate, 2),
      "sec/°F (95% CI:",
      round(elite_ci$conf.low, 2),
      "to",
      round(elite_ci$conf.high, 2),
      "sec/°F)\n"
    )
    
    Residuals vs fitted plot for the general sample
    
    augment(general_model) %>%
    ...
    #Adjusted R^2:
    glance(general_model)$adj.r.squared
    glance(elite_model)$adj.r.squared
    
    Combining datasets
    
    interaction_data <- bind_rows(
      final_data %>% mutate(group = "General"),
      top_1percent %>% mutate(group = "Elite")
    )
    
    Interaction regression: Time ~ Temperature*Group
    
    interaction_model <- lm(finish_time_seconds ~ avg_race_temp_f * group, data = interaction_data)
    ...
    #with changes: commit, publish in Github | then should be in cloudconnect, add tabs then can publish each of them
    
    1. View the 95% Confidence Intervals for General and Elite models
    
    tidy(general_model, conf.int = TRUE, level = 0.95)
    tidy(elite_model, conf.int = TRUE, level = 0.95)
    
    2. View Adjusted R-Squared values
    
    cat("General Adj R2:", glance(general_model)$adj.r.squared, "\n")
    cat("Elite Adj R2:", glance(elite_model)$adj.r.squared, "\n")
    
    3. View the Interaction Model results (to compare elite vs general slopes formally)
    ...
    Print Interaction Model to test if elite vs general slopes differ significantly
...
ui <- navbarPage(
  ...
  tabPanel("Overall Thermal Sensitivity",
           sidebarLayout(
             sidebarPanel(
               selectInput(
                 inputId = "group",
             ...
             mainPanel(
               plotlyOutput("scatter_plot_overall"),
               br(),
  ...
  tabPanel("Quartile Sensitivity",
           sidebarLayout(
             ...
             mainPanel(
               plotlyOutput("scatter_plot_quartile"),
               br(),
               tableOutput("summary_table_quartile")
             )
           )
  )
)

STREAMING_CHUNK: Defining the Unified Shiny Server logic...

server <- function(input, output, session) {
  
  --- Logic for Tab 1 (Overall) ---
    
    yearly_data_overall <- reactive({
      spread_data %>%
    ...
    output$scatter_plot_overall <- renderPlotly({
      p <- ggplot(yearly_data_overall(), aes(x = avg_temp, y = avg_time_hours)) +
    ...
    output$summary_table_overall <- renderTable({
      spread_data %>%
        filter(runner_group == input$group) %>%
        summarize(
    ...
    --- Logic for Tab 2 (Quartiles) ---
      
      yearly_data_quartile <- reactive({
        final_data %>%
    ...
    output$scatter_plot_quartile <- renderPlotly({
      p <- ggplot(yearly_data_quartile(), aes(x = avg_temp, y = avg_time_hours)) +
    ...
    output$summary_table_quartile <- renderTable({
      final_data %>%
        filter(quartile == input$quartile_choice) %>%
        summarize(
          Overall Avg Time (Hours) = mean(finish_time_seconds)/3600,
...
STREAMING_CHUNK: Launching the Shiny App...
